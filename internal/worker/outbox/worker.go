...
func (w *Worker) processSubmissionTest(ctx context.Context, event *models.OutboxEvent) error {
	// Parse payload
	...
	tests := make([]models.ProblemTest, len(testRows))
	for i, t := range testRows {
		tests[i] = models.ProblemTest{
			ID:        t.ID,
	...
	w.logger.Info("submission tested successfully",
		...
		slog.Int64("score", int64(results.Score)))
...
func (w *Worker) publishSubmissionCreated(submission *models.Submission) {
...
func (w *Worker) publishSubmissionUpdated(submission *models.Submission) {
...
func (w *Worker) buildSubmissionWebSocketEvent(submission *models.Submission, messageType models.WebSocketMessageType) *models.SubmissionWebSocketEvent {
	// Convert pgtype.UUID to uuid.UUID safely
	var userID, problemID, contestID uuid.UUID
	if submission.CreatedBy != nil {
	...
	if submission.Position != nil {
		position = int32(*submission.Position)
	}
	...
	return &models.SubmissionWebSocketEvent{
		...
		Submission: &models.SubmissionListItem{
			...
			TimeStat:     submission.TimeStat,
			MemoryStat:   submission.MemoryStat,
...
func (w *Worker) buildSubmissionContext(submission *models.Submission) *SubmissionContext {
	var userID, problemID, contestID uuid.UUID
	if submission.CreatedBy != nil {
		userID = *submission.CreatedBy
	}
...
func (w *Worker) testSubmission(
	...
	for i, test := range tests {
		...
		if result.Memory != nil {
			...
			memoryMB := int32(*result.Memory / 1024)
	...
	if totalTests > 0 {
		results.Score = int32(passedTests * 100 / totalTests)
	}
