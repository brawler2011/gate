...
import type { ContestModel, OrganizationMemberModel, ProblemsListItemModel, TeamModel } from '@contracts/gateway/v1';
import { Tabs } from '@mantine/core';
import { IconPuzzle, IconTrophy, IconUsers, IconUsersGroup } from '@tabler/icons-react';
import type { ApiError } from '@/lib/api';
import { useRouter, useSearchParams } from 'next/navigation';
import { useCallback } from 'react';

const VALID_TABS = ['members', 'teams', 'problems', 'contests'] as const;
...
export function OrgTabs({ members, teams, problems, contests, orgId, membersError, teamsError, problemsError, contestsError }: Props) {
  ...
  return (
    <Tabs value={activeTab} onChange={handleTabChange} keepMounted={false}>
      ...
      <Tabs.Panel value="members" pt="md">
        {membersError ? <ErrorDisplay error={membersError} /> : <OrgMembersTab members={members} />}
      </Tabs.Panel>
      <Tabs.Panel value="teams" pt="md">
        {teamsError ? <ErrorDisplay error={teamsError} /> : <OrgTeamsTab teams={teams} orgId={orgId} />}
      </Tabs.Panel>
      <Tabs.Panel value="problems" pt="md">
        {problemsError ? <ErrorDisplay error={problemsError} /> : <OrgProblemsTab problems={problems} />}
      </Tabs.Panel>
      <Tabs.Panel value="contests" pt="md">
        {contestsError ? <ErrorDisplay error={contestsError} /> : <OrgContestsTab contests={contests} />}
      </Tabs.Panel>
    </Tabs>
  );
}
