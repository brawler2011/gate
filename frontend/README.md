...
- **Управление пользователями**: Встроенная аутентификация, роли (admin/user), управление правами доступа к контестам
...
- **Core API**: Единый клиент для всех микросервисов (`@contracts/core/v1`)
- Routing через NGINX к соответствующим бэкенд-сервисам (blogs, core/tester)
- Type-safe методы и модели

### Аутентификация

- **Custom Session Auth**: Встроенная аутентификация на основе сессий
- Session-based authentication с cookies (`session_id`)
...
- **@contracts/core/v1**: Сгенерированный OpenAPI клиент (см. `/contracts`)
...
- **Infrastructure**: `/infr` - Docker Compose, Nginx configs
