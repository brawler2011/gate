...
func (uc *JudgeUseCase) compileComponents(ctx context.Context, g *gfmt.GateFormat, problemID uuid.UUID) (map[string]sandbox.Executable, error) {
	...
	for compType, folderName := range folders {
		dirPath := filepath.Join(g.Path, folderName)
		...
		for _, entry := range entries {
			if !entry.IsDir() && (strings.HasSuffix(entry.Name(), ".cpp") || strings.HasSuffix(entry.Name(), ".py") || strings.HasSuffix(entry.Name(), ".go")) {
				filename = entry.Name()
				break
			}
		}
